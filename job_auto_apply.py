...
def pick_best_resume(iterations: List[IterationResult]) -> IterationResult:
...
def check_sponsorship_viability(description: str) -> bool:
    ...
    for phrase in blocklist:
        if phrase in description:
            # Check for "unless" exceptions? (Rare, usually denial is absolute in these phrases)
            return False
...
CITIZENSHIP_PATTERNS = [
    ...
    r"\bno visa sponsorship\b", r"\bnot eligible for visa sponsorship\b",
    r"\bwithout sponsorship\b", r"\bf1-?opt not accepted\b",
    r"\bgreen card holders? only\b", r"\bus citizen or green card\b",
]
...
def render_resume_from_template(json_data: dict, template_path: str) -> str:
    """Render LaTeX resume from Jinja2 template and JSON data"""
...
def main():
    ...
    parser.add_argument(
        "--out_dir",
        default=str(Path.home() / "Desktop" / "Google Auto"),
...
def pick_best_resume(all_iterations: list) -> dict:
    """
    Compare all iterations and pick the one with best interview probability.
    Called when max iterations reached without approval.
    """
    print("   ⚠️  Max iterations reached without approval")
    print("   📊 Comparing all iterations to pick best...")
    
    # Sort by score (higher is better)
    # Sort by score (higher is better)
    sorted_iterations = sorted(all_iterations, key=lambda x: x.get('score', 0) if isinstance(x, dict) else getattr(x, 'score', 0), reverse=True)
    best = sorted_iterations[0]
    
    print(f"   ✅ Selected iteration #{best['iteration']} (score: {best['score']})")
    print(f"      Reason: Highest interview probability")
...
def pick_best_resume(all_iterations: list) -> dict:
    ...
    # Sort by score (higher is better)
    sorted_iterations = sorted(all_iterations, key=lambda x: x.get('score', 0), reverse=True)
    best = sorted_iterations[0]
